...
public class MoreThanNdK {
	...
	public static void moreThanNdK(int arr[], int n, int k) {
		...
		for (int i = 0; i < n; i++) {
			...
			/*
			 * If arr[i] is already present in the element count array, then
			 * increment its count
			 */
			for (j = 0; j < k - 1; j++) {
				if (temp[j].e == arr[i]) {
			...
			if (j == k - 1) {
				...
				/*
				 * If there is position available in temp[], then place arr[i]
				 * in the first available position and set count as 1
				 */
				for (l = 0; l < k - 1; l++) {
					if (temp[l].c == 0) {
						temp[l].e = arr[i];
