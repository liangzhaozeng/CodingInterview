...
public class MedianOfTwoSortedArrays {
	...
	/**
	 * O(log(n + m)) time, O(1) space. Each time remove a half of an array
	 */
	public static class Solution {
		public double findMedianSortedArrays(int a[], int b[]) {
			...
			if ((total % 2) == 1) {
				...
				return (findKth(a, 0, m - 1, b, 0, n - 1, total / 2) + findKth(a, 0, m - 1, b, 0, n - 1, total / 2 + 1)) / 2;
			}
		}
		...
		private double findKth(int[] A, int leftA, int rightA, int[] B, int leftB, int rightB, int k) {
			...
			// number of elements smaller than or equal to the larger of A[midA]
			// and B[midB]
