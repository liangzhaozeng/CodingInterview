...
public class ConstructBinaryTreeFromInorderAndPostorderTraversal {
	...
	public class Solution2 {
		...
		TreeNode build(int[] inorder, int startI, int endI, int[] postorder,
				int startP, int endP) {
			...
			TreeNode left = build(inorder, startI, indxOrder - 1, postorder,
					startP, endP - rest - 1);
			TreeNode right = build(inorder, indxOrder + 1, endI, postorder,
					endP - rest, endP - 1);
			parent.left = left;
			parent.right = right;
			return parent;
	...
	public class Solution {
		...
		public TreeNode buildTree(int[] inorder, int inFrom, int inTo,
			...
			root.left = buildTree(inorder, inFrom, rootInorderIdx - 1,
					postorder, postFrom, postLeftEnd);
			root.right = buildTree(inorder, rootInorderIdx + 1, inTo,
					postorder, postLeftEnd + 1, postTo - 1);
